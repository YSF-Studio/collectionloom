...
/// Start network capture using pcap (Unix). Windows requires Npcap runtime separately.
#[cfg(unix)]
pub fn start_capture(
    ...
    loop {
        ...
        if config.max_duration_secs > 0 && started.elapsed().as_secs() >= config.max_duration_secs {
            break;
        }
...
pub fn start_capture(
    ...
    Err("Live packet capture is not available on this platform in CI builds. Install Npcap on Windows or use libpcap on Linux/macOS.".into())
}

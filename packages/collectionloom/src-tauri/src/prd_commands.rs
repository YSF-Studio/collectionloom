...
pub fn open_in_analysisloom(case_id: String) -> Result<String, String> {
    ...
    let handoff = serde_json::json!({
        "schema": "collectionloom-handoff-v1",
        "case_id": case_id,
        "case_title": case.title,
        "case_dir": case_dir(&case_id).to_string_lossy(),
        "export_dir": export_dir.to_string_lossy(),
        "evidence_pack": if pack.exists() { pack.to_string_lossy().into_owned() } else { String::new() },
        "opened_at": Utc::now().to_rfc3339(),
    });
    let handoff_path = write_handoff(&case_id, &handoff)?;

    let readme = export_dir.join("ANALYSISLOOM_HANDOFF.md");
    let md = format!(
        "# CollectionLoom → AnalysisLoom Handoff\n\n\
         Case: **{}** (`{case_id}`)\n\n\
         1. Open AnalysisLoom\n\
         2. Import case from: `{}`\n\
         3. Or load disk image / evidence pack from this folder\n\n\
         Handoff manifest: `{}`\n",
        case.title,
        export_dir.display(),
        handoff_path.display()
    );
    fs::write(&readme, md).map_err(|e| e.to_string())?;

    #[cfg(target_os = "macos")]
    {
        let apps = [
            "/Applications/AnalysisLoom.app",
            "/Applications/YSF AnalysisLoom.app",
        ];
        for app in apps {
            if std::path::Path::new(app).exists() {
                std::process::Command::new("open")
                    .args(["-a", app, handoff_path.to_str().unwrap_or("")])
                    .spawn()
                    .map_err(|e| e.to_string())?;
                return Ok(format!("Opened {app} with handoff manifest"));
            }
        }
        std::process::Command::new("open")
            .arg(&export_dir)
    ...
    Ok(format!(
        "Handoff ready at {} — open AnalysisLoom and import this case folder",
        export_dir.display()
    ))
}
